...
int main(int argc, char* argv[]) {
    ...
    if (options.Lx <= 0 || options.Ly <= 0 || options.Lz <= 0) {
        spdlog::error("Invalid lattice size Lx={} Ly={} Lz={}", options.Lx, options.Ly, options.Lz);
        return 1;
    }
    ...
    auto options_stream = std::fstream(
        make_file_path(options.data_dir, "opt", options.Lx, options.Ly, options.Lz, options.name, "yaml"),
        std::ios::out
    );
    ...
    auto options_toml_stream = std::fstream(
        make_file_path(options.data_dir, "opt", options.Lx, options.Ly, options.Lz, options.name, "toml"),
        std::ios::out
    );
    ...
    lattice_t lat({options.Lx, options.Ly, options.Lz}, 'C');
    lft::ea::JLattice<lattice_t> j_lat({3, lat.dims[0], lat.dims[1], lat.dims[2]}, 'C');
    ...
    auto j_path = make_file_path(options.data_dir, "j", options.Lx, options.Ly, options.Lz, options.name, "txt");
    ...
    *********************/
    ...
    for (int i = 0; i < options.n_betas(); ++i) {
        em_stream_ptrs[i] = optional_fstream_ptr(
            make_file_path(options.data_dir, "em", options.Lx, options.Ly, options.Lz,
    ...
    for (int i = 0; i < options.n_betas(); ++i) {
        cfg_stream_ptrs[i] = optional_fstream_ptr(
            make_file_path(options.data_dir, "cfg", options.Lx, options.Ly, options.Lz,
                           options.name + std::format("_b{:03d}", i),
                           "bin"),
    ...
    if (options.exchange_freq > 0 && options.n_sweeps > 0) {
        temperer.print_stats(std::cout);
        auto exch_file = make_file_path(options.data_dir, "exch", options.Lx, options.Ly, options.Lz, options.name,
                                        "txt");
